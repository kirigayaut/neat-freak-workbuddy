...
| 层 | 位置 | 写入规则 |
|---|---|---|
| 规则层 | 项目根 `AGENTS.md` | 可就地编辑。WorkBuddy 原生加载 AGENTS.md；项目里同时存在 CLAUDE.md 时，只在项目自己声明同源时才对齐，否则只报告分叉 |
...
**与 WorkBuddy 内建记忆机制的关系**：WorkBuddy 在完成实质工作后会自动追加当日日记、必要时更新 MEMORY.md。洁癖不是另起炉灶，而是这个机制的收尾审查版：负责跨层对齐、superseded 标注、到期蒸馏和矛盾裁决。同一事实已由当次会话正确写入记忆的，不重复记录；发现写错位置或已过期时才动手修正。
