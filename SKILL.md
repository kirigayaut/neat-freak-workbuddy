...
description: >-
  ...
  (AGENTS.md), WorkBuddy project memory (.workbuddy/memory/), and workspace
  residue with what the code and runtime actually do, so the next session or
  the next person starts from one current answer. Trigger when the user names
  ...
  stale or conflicting AGENTS.md/memory, a clean handoff to a teammate or a
...
compatibility: Adapted for WorkBuddy. Requires filesystem read access. Writes and destructive actions follow the active agent, workspace, and user authorization rules. Git and rg improve verification; scripts/audit-inventory.sh needs Bash — without it, do the equivalent checks manually.
metadata:
  version: "3.0.0-wb.1"
  category: knowledge-governance
  upstream: "https://github.com/KKKKhazix/khazix-skills/tree/main/neat-freak (MIT, v3.0.0)"
...
3. **落实现役事实（不碰 AGENTS.md）**：项目有可运行代码、但下次会话恢复上下文需要的五件事（一句话定位、怎么跑起来、技术栈、目录与约定、当前状态和下一步）没有稳定落点时，**写进 `.workbuddy/memory/MEMORY.md` 的「项目现役事实」段落**——这是 WorkBuddy 里唯一保证每次会话自动生效的通道，控制在 30 行内。这一步不创建也不修改 AGENTS.md；项目里已有 AGENTS.md 时只读它，发现矛盾记进汇报的「待你确认」，不顺手改。
...
## WorkBuddy 平台事实（本适配版的核心约定）

在 WorkBuddy 环境运行时，知识层的落位和写入规则固定如下，优先级高于上表的通用描述：

| 层 | 位置 | 写入规则 |
|---|---|---|
| 规则层 | 项目根 `AGENTS.md` | **既不主动新建，也不日常维护**——它对 WorkBuddy 自身工作没有影响。只在两种时刻生成：用户要拿别的 Agent 跑这个项目，或明确要求交接（见下）。已经存在的文件默认**只读**：盘点时读它，发现与现役事实冲突就报出来、列成「重写 / 删除候选」，不擅自修改；用户当场说「顺手修一下」才动手 |
| 记忆层-日记 | `{项目}/.workbuddy/memory/YYYY-MM-DD.md` | **只追加，不回改历史**。发现日记里有过期事实时，不改写旧行，而是在当日日记或文件末尾追加一行标注：`> superseded 2026-09-15: <旧事实> → <现役事实>（依据）` |
| 记忆层-蒸馏 | `{项目}/.workbuddy/memory/MEMORY.md` | 可就地编辑的长期项目笔记，**单会话净写入 ≤3000 字符**。「删除优于保留」「合并优于追加」只在这一层充分适用 |
| 用户级记忆 | `~/.workbuddy/MEMORY.md` | 跨项目偏好，≤4000 字符；只写用户明确要求长期记住的非项目特定内容 |
| 日记维护 | 超过 30 天的日记 | 按主题蒸馏进项目 `MEMORY.md` 后，旧日记文件可删除；删除前必须列进汇报的删除候选清单，经用户确认 |

**禁区（不适用「可直接修」，发现即只读报告）：**

- `.workbuddy/` 目录本身不是缓存：除 `memory/` 按上表维护外，其结构和其它子目录（skills/、automations 数据等）禁止删除、清空或移动。
- WorkBuddy 自动化任务（automations）只能通过自动化管理接口增删改，禁止用文件操作触碰。
- 个人目录（Desktop / Downloads / Documents / Home 等非项目目录）适用个人文件安全规则：洁癖的清场和删除候选永远不涵盖它们；扫描请求只出报告。
- 「做完后清理」的预授权不替代汇报后的确认（与上游清场 gate 一致）。

**与 WorkBuddy 内建记忆机制的关系**：WorkBuddy 在完成实质工作后会自动追加当日日记、必要时更新 MEMORY.md，并在每次会话开始自动读取记忆层——这是 WorkBuddy 里唯一保证跨会话生效的通道。**与 WorkBuddy 内建记忆机制的关系**：WorkBuddy 在完成实质工作后会自动追加当日日记、必要时更新 MEMORY.md，并在每次会话开始自动读取记忆层——这是 WorkBuddy 里唯一保证跨会话生效的通道。因此收尾时的默认落点是：下次会话必须生效的关键约束（红线、现役事实）写进项目 `MEMORY.md`，不往 AGENTS.md 里写。

### 交接模式：什么时候才生成 AGENTS.md

只在下面两种时刻动手，其余一律不动：

1. **用户要把项目交给别的 Agent 跑**（说「这个项目我要用 Codex / Claude Code / Cursor 跑」「给别的 Agent 接手」等）；
2. **用户明确要求交接**（说「生成一份 AGENTS.md」「写个交接文档给同事」等）。

辅助判据（不独立触发，只用来主动问一句）：项目里出现 `.claude/`、`.codex/`、`.cursor/`、`.windsurf/` 等其他 Agent 的配置目录，而项目又没有 AGENTS.md。

生成时的做法：

- 从项目 `MEMORY.md`、docs/、README 和当次会话上下文**合成**一份，不是翻译其中任意一份；五要素齐、≤60 行。
- 顶部写清生成日期，并注明「自动生成，供其他 Agent / 接手者使用，WorkBuddy 侧不自动维护」——让下一个读者知道它的新鲜度边界。
- 已存在且过时：先给「重写 / 删除候选」让用户选，确认后才覆盖。

洁癖不是另起炉灶，而是这个机制的收尾审查版：负责跨层对齐、superseded 标注、到期蒸馏和矛盾裁决。同一事实已由当次会话正确写入记忆的，不重复记录；发现写错位置或已过期时才动手修正。
...
- **WorkBuddy 记忆视为已授权维护面**，但写入方式固定：日记（`.workbuddy/memory/YYYY-MM-DD.md`）只追加和加 superseded 标注，不回改；蒸馏层（`MEMORY.md`）就地编辑并遵守 3000 字符/会话预算；用户级 `~/.workbuddy/MEMORY.md` 只放跨项目偏好。不要把临时检索结果、工具报错、一次性路径写进任何一层。
...
- [ ] 轻量路径：五要素已落进 `MEMORY.md`；未凭空新建也没顺手修改 AGENTS.md（已有文件的冲突已进汇报，未擅自改）；残留清单已交用户确认，未确认未删。
