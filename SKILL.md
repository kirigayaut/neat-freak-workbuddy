...
3. **落实现役事实（规则文件降级为可选）**：项目有可运行代码、但下次会话恢复上下文需要的五件事（一句话定位、怎么跑起来、技术栈、目录与约定、当前状态和下一步）没有稳定落点时，**写进 `.workbuddy/memory/MEMORY.md` 的「项目现役事实」段落**——这是 WorkBuddy 里唯一保证每次会话自动生效的通道，控制在 30 行内。不要为了「应该有份规则文件」去新建 `AGENTS.md`：项目里已存在 AGENTS.md 就只修矛盾和过期项，不推倒重写；只有用户明确说这个仓库也要给其他 Agent / 同事用时，才按 60 行内的最小规则文件创建它。
...
| 层 | 位置 | 写入规则 |
|---|---|---|
| 规则层 | 项目根 `AGENTS.md` | **默认不新建**。它是否会被自动加载无法保证，凭空生成一份没人读的文件就是在制造新的过期源。只有三种情况动它：项目里已存在（只修矛盾和过期项，不推倒重写）；用户明确说这个仓库也要给其他 Agent / 同事用（那时按 60 行内最小规则文件创建）；**检测到项目里已有其他 Agent 的配置目录（`.claude/`、`.codex/`、`.cursor/`、`.windsurf/` 等）而项目又没有 AGENTS.md 时，主动问一句要不要生成**——这是「别的 Agent 可能要接手」的现场证据。生成后它就进入维护轨道。 |
...
**与 WorkBuddy 内建记忆机制的关系**：WorkBuddy 在完成实质工作后会自动追加当日日记、必要时更新 MEMORY.md，并在每次会话开始自动读取记忆层——这是 WorkBuddy 里唯一保证跨会话生效的通道。因此收尾时的默认落点是：**下次会话必须生效的关键约束（红线、现役事实）写进项目 `MEMORY.md`**；项目已有 AGENTS.md 时，两处保持同源（AGENTS.md 留短指针，不复制成第二份全文）。洁癖不是另起炉灶，而是这个机制的收尾审查版：负责跨层对齐、superseded 标注、到期蒸馏和矛盾裁决。同一事实已由当次会话正确写入记忆的，不重复记录；发现写错位置或已过期时才动手修正。
...
- [ ] 轻量路径：五要素已落进 `MEMORY.md`（或项目既有的规则文件）；未凭空新建没人读的 AGENTS.md；残留清单已交用户确认，未确认未删。
