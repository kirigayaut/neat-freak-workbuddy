...
3. **补 AI 规则文件**：项目有可运行代码但没有任何规则文件时，默认创建一份最小规则文件（WorkBuddy 环境固定用 `AGENTS.md`；其他平台按其原生名字），只写五件事：项目一句话定位、怎么跑起来、技术栈、目录与约定、当前状态和下一步。控制在 60 行内——这份文件是下次会话恢复上下文的入口，不是第二份 README。已有规则文件则只修矛盾和过期项，不推倒重写。
...
| 层 | 位置 | 写入规则 |
|---|---|---|
| 规则层 | 项目根 `AGENTS.md` | 可就地编辑。WorkBuddy 确定自动注入的是记忆体系与身份文件（SOUL/IDENTITY/USER）；AGENTS.md 是官方推荐的工作区规则文件，但是否每次会话自动加载以现场为准——收尾时先验证（看新会话是否引用其内容），不确定就在汇报里注明 |
...
**与 WorkBuddy 内建记忆机制的关系**：WorkBuddy 在完成实质工作后会自动追加当日日记、必要时更新 MEMORY.md，并在每次会话开始自动读取记忆层——这是 WorkBuddy 里唯一保证跨会话生效的通道。因此收尾时有一条兜底规则：**下次会话必须生效的关键约束（红线、现役事实），除了写进 AGENTS.md，还要在项目 `MEMORY.md` 里留一条短指针**；AGENTS.md 未被自动加载时，记忆层仍能兜住。洁癖不是另起炉灶，而是这个机制的收尾审查版：负责跨层对齐、superseded 标注、到期蒸馏和矛盾裁决。同一事实已由当次会话正确写入记忆的，不重复记录；发现写错位置或已过期时才动手修正。
