...
for dir in "$HOME/.claude" "$HOME/.codex" "$HOME/.cursor" "$HOME/.gemini" \
  "$HOME/.qoder" "$HOME/.trae" "$HOME/.iflow" "$HOME/.codebuddy" "$HOME/.workbuddy"; do
  [[ -d "$dir" ]] && printf '%s\n' "$dir"
done

section "workbuddy-memory"
# WorkBuddy project memory: metadata only (daily logs are append-only; MEMORY.md is the curated layer).
if [[ -d "$PROJECT_ROOT/.workbuddy/memory" ]]; then
  find "$PROJECT_ROOT/.workbuddy/memory" -maxdepth 1 -type f -name '*.md' | LC_ALL=C sort | while IFS= read -r f; do
    file_size "$f"
  done
  old_count="$(find "$PROJECT_ROOT/.workbuddy/memory" -maxdepth 1 -type f -name '*.md' -mtime +30 ! -name 'MEMORY.md' | wc -l | tr -d ' ')"
  printf 'daily_logs_older_than_30d=%s\n' "$old_count"
else
  printf 'none\n'
fi
[[ -f "$HOME/.workbuddy/MEMORY.md" ]] && file_size "$HOME/.workbuddy/MEMORY.md"
