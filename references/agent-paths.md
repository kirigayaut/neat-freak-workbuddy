...
| 用途 | 路径 / 规则 |
|---|---|
| 项目规则（交接用） | `{项目}/AGENTS.md`——默认**不新建、不维护**（对 WorkBuddy 自身无影响）；仅在用户要交给别的 Agent 或明确要求交接时生成；已存在的文件默认只读，冲突只报告 |
| 项目记忆-日记 | `{项目}/.workbuddy/memory/YYYY-MM-DD.md`，**只追加**，过期内容用 `> superseded <日期>: ...` 追加标注 |
| 项目记忆-蒸馏 | `{项目}/.workbuddy/memory/MEMORY.md`，可编辑，单会话净写入 ≤3000 字符 |
| 用户级记忆 | `~/.workbuddy/MEMORY.md`，跨项目偏好，≤4000 字符 |
| 用户级 Skills | `~/.workbuddy/skills/<name>/SKILL.md` |
| 项目级 Skills | `{项目}/.workbuddy/skills/<name>/SKILL.md` |
| 身份文件 | `~/.workbuddy/SOUL.md`、`IDENTITY.md`、`USER.md`（人工维护，审计时只读报告） |

WorkBuddy 专属口径：

- 日记超过 30 天：按主题蒸馏进项目 `MEMORY.md`，旧日记文件列入删除候选，经用户确认后删除。
- `.workbuddy/` 不是缓存目录：除 `memory/` 按上述规则维护外，禁止删除或重构；automations 只能走自动化管理接口。
- 项目里发现 `CLAUDE.md` 时按「其他平台产物」处理：只报告分叉风险，不擅自搬运或删除。
- 归类三分法在 WorkBuddy 下的落位：AGENTS.md = 人工规则；`memory/*.md` = Agent 自动记忆（日记追加型 + 人工蒸馏层）；`.workbuddy/` 下其余文件 = 机器生成，默认只读。
